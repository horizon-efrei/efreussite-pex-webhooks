<h1 align="center">Ef'Réussite PEX Webhooks</h1>
<p align="center">
...
1. Ajouter un fichier markdown dans [`./resources`](/resources). Choisissez un des sous-dossiers qui correspond à votre message.
    ...
    À part pour `guide` et `rules`, il vous faudra choisir un deuxième sous-dossier correspondant à l'année scolaire en cours. Par exemple, `bachelor/2022-2023/`.
...
- Utilisez des images ! Pour cela, il faut :
    1. Créez un ou des **dossier(s)** dans `./resources/_images/` correspondants aux dossiers dans lesquels vous avez mis votre annonce. Exemples :
         ...
         - Pour une annonce dans `./resources/bachelor/2021-2022/Ateliers mobilité S5.md`, il faut créer le dossier `./resources/_images/bachelor/2021-2022/Ateliers mobilité S5/`
...
1. Allez sur le workflow GitHub [de déploiement](https://github.com/horizon-efrei/efreussite-pex-webhooks/actions/workflows/deployment.yml) ou [de pré-déploiement](https://github.com/horizon-efrei/efreussite-pex-webhooks/actions/workflows/draft-deployment.yml) pour tester d'abord.
1. Cliquez sur "Run workflow".
1. Dans le champs d'input, mettez le nom du fichier à déployer avec ses dossiers parents, sauf le dossier `resource`. Exemple : `guide/Premier message.md` ou `bachelor/2021-2022/Ateliers mobilité S5` (le `.md` est facultatif).
1. Confirmez en cliquant sur "Run workflow".

#### ✏️ Mise à jour d'un message

La mise à jour d'un message se fait manuellement.
Installez le projet en local avec `git clone https://github.com/horizon-efrei/efreussite-pex-webhooks && cd efreussite-pex-webhooks && npm install`. Si la commande `npm` n'est pas reconnue, vous devez [installer Node.js](https://nodejs.org/en/download/).
